  ...
  return (
    <div className="mx-auto max-w-xl space-y-8">
      ...
      <section className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
        <div className="mb-4 flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgba(255,107,0,0.15)] ring-1 ring-[rgba(255,107,0,0.3)]"><Eye className="h-4 w-4 text-[rgb(var(--accent-orange))]" /></div><h2 className="text-sm font-semibold text-foreground">Display</h2></div>
        <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-black/20 px-4 py-3"><span className="text-sm text-foreground">Show heatmap overlay</span><input type="checkbox" checked={s.showHeatmap} onChange={(e) => setS({ ...s, showHeatmap: e.target.checked })} className="h-4 w-4 rounded border-white/20 accent-[rgb(var(--accent-orange))]" /></label>
      </section>
      ...
      <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
        <div className="flex items-center justify-between"><div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgba(255,107,0,0.15)] ring-1 ring-[rgba(255,107,0,0.3)]"><Mail className="h-4 w-4 text-[rgb(var(--accent-orange))]" /></div><h2 className="text-sm font-semibold text-foreground">Email alerts</h2></div><label className="flex cursor-pointer items-center gap-2 text-xs text-muted"><input type="checkbox" checked={s.emailEnabled} onChange={(e) => setS({ ...s, emailEnabled: e.target.checked })} className="h-4 w-4 rounded border-white/20 accent-[rgb(var(--accent-orange))]" />Enable</label></div>
      ...
      <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
        <div className="flex items-center justify-between"><div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgba(255,107,0,0.15)] ring-1 ring-[rgba(255,107,0,0.3)]"><MessageSquare className="h-4 w-4 text-[rgb(var(--accent-orange))]" /></div><h2 className="text-sm font-semibold text-foreground">Telegram alerts</h2></div><label className="flex cursor-pointer items-center gap-2 text-xs text-muted"><input type="checkbox" checked={s.telegramEnabled} onChange={(e) => setS({ ...s, telegramEnabled: e.target.checked })} className="h-4 w-4 rounded border-white/20 accent-[rgb(var(--accent-orange))]" />Enable</label></div>
      ...
      <div className="flex flex-wrap gap-3">
        <button type="button" onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[rgb(var(--accent-orange))] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_rgba(255,107,0,0.35)] transition hover:brightness-110 hover:shadow-[0_0_28px_rgba(255,107,0,0.5)] disabled:opacity-50 disabled:shadow-none">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save settings</button>
        <button type="button" onClick={testNotifications} disabled={testing} className="inline-flex items-center gap-2 rounded-xl border border-white/[0.15] px-5 py-2.5 text-sm font-medium text-foreground transition hover:bg-white/[0.06] hover:border-white/25 disabled:opacity-50">{testing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Test notifications</button>
      </div>
    </div>
  );
}
