    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" aria-label="Notifications" className="relative inline-flex h-11 w-11 items-center justify-center rounded-xl bg-white/[0.05] text-foreground/80 ring-1 ring-white/10 transition hover:bg-white/10 hover:text-foreground">
            <Bell className="h-5 w-5" strokeWidth={1.75} />
            <span className="absolute right-2.5 top-2.5 dot-orange" />
          </button>
          <button type="button" aria-label="Profile" className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[rgba(255,107,0,0.18)] text-[rgb(var(--accent-orange))] ring-1 ring-[rgba(255,107,0,0.35)] transition hover:bg-[rgba(255,107,0,0.28)]">
      ...
      <section className="space-y-4">
        ...
        {faces.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {faces.map((f) => (
              <div key={f.id} className="relative flex items-center gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.04] p-4 backdrop-blur-xl transition hover:bg-white/[0.07]">
