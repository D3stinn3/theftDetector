...
export default function LiveFeeds() {
  ...
  return (
    <div className="space-y-4">
      ...
      {alert && (
        <div
          className="flex items-start gap-3 rounded-2xl border border-[rgb(255,77,0)]/30 bg-[rgb(255,77,0)]/[0.08] px-4 py-4 backdrop-blur-xl"
          role="alert"
        >
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-[rgb(255,77,0)]">
              Theft Alert
            </p>
      ...
      <div className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-2.5">
        <div className="flex items-center gap-2.5">
          ...
          >
            {connected && (
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[rgb(0,255,190)] opacity-50" />
            )}
            <span
              className={`relative inline-flex h-2.5 w-2.5 rounded-full ${
                connected ? "bg-[rgb(0,255,190)] shadow-[0_0_8px_rgba(0,255,190,0.7)]" : "bg-muted/40"
              }`}
            />
          </span>
      ...
      {feeds.length === 0 ? (
        ...
        >
          {feeds.map((cam) => (
            <div
              key={cam.camera_id}
              className="overflow-hidden rounded-2xl border border-white/[0.08] bg-black/40 shadow-2xl shadow-black/40 backdrop-blur-xl transition hover:border-white/[0.14]"
            >
              {/* Camera header */}
              <div className="flex items-center justify-between border-b border-white/[0.06] bg-black/30 px-3 py-2.5">
                ...
                <span className="rounded-md bg-white/[0.06] px-1.5 py-0.5 font-mono text-[10px] text-muted">
