      ...
      <header className="flex items-start justify-between">
        ...
        {rows.length > 0 && (
          <div className="flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 py-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-[rgb(255,77,0)]" />
            <span className="text-xs font-medium text-foreground">{rows.length} events</span>
          </div>
        )}
      </header>
      ...
      {rows.length === 0 ? (
        ...
        <ul className="space-y-3">
          {rows.map((row) => (
            <li
              key={row.id}
              className="group flex flex-col gap-4 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 backdrop-blur-xl transition hover:bg-white/[0.05] hover:border-white/[0.12] sm:flex-row sm:items-start"
            >
              {/* Left accent bar */}
              <div className="absolute left-0 top-0 h-full w-0.5 rounded-l-2xl bg-[rgb(255,77,0)] opacity-60" />
              ...
              <div className="relative shrink-0">
                ...
                <img
                  ...
                  alt=""
                  className="h-28 w-40 rounded-xl border border-white/[0.08] object-cover"
                />
