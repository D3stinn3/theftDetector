    ...
    <div className="mx-auto max-w-2xl space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted">
          USB index (e.g.{" "}
          <code className="rounded-lg bg-white/[0.07] px-1 py-0.5 text-foreground">0</code>
          ) or RTSP URL. Select which cameras auto-load on backend startup.
        </p>
      </header>
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgba(255,107,0,0.15)] ring-1 ring-[rgba(255,107,0,0.3)]">
                <Save className="h-4 w-4 text-[rgb(var(--accent-orange))]" />
              </div>
              <div>
                <h2 className="text-sm font-semibold text-foreground">Startup cameras</h2>
                <p className="text-xs text-muted">
                  Saved as{" "}
                  <code className="rounded bg-white/[0.07] px-1 py-0.5 text-foreground">cameraSources</code>{" "}
                  in settings.json
                </p>
              </div>
            </div>
            ...
            {startup.length === 0 ? (
              <p className="rounded-xl border border-dashed border-white/[0.08] bg-black/10 px-4 py-3 text-sm text-muted">
                No startup cameras saved yet. Select cameras below and click "Save as startup cameras".
              </p>
            ) : (
                ...
                {startup.map((c, i) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate font-mono text-xs text-muted">{c.source}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-[rgb(var(--accent-orange))] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_rgba(255,107,0,0.35)] transition hover:brightness-110 hover:shadow-[0_0_28px_rgba(255,107,0,0.5)] disabled:opacity-50 disabled:shadow-none"
            >
              {savingStartup ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
          ...
          {cams.length > 0 && (
            <ul className="space-y-2">
              {cams.map((c) => {
                const online = c.status === "open" || c.status === "running" || c.status === "active";
                return (
                  <li
                    key={c.id}
                    className="flex items-center justify-between gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3.5 backdrop-blur-xl transition hover:bg-white/[0.05]"
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      {/* Camera icon badge */}
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/[0.06] ring-1 ring-white/[0.08]">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span
                            ...
                            style={{
                              background: online ? "rgba(0,255,190,0.12)" : "rgba(110,120,145,0.15)",
                              color: online ? "rgb(0,255,190)" : "rgb(110,120,145)",
                              border: online ? "1px solid rgba(0,255,190,0.25)" : "1px solid rgba(110,120,145,0.2)",
                            }}
                          >
                    ...
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-2 text-xs text-muted">
                        <input
                          ...
                          onChange={(e) =>
                            setStartupSelected((prev) => ({ ...prev, [c.id]: e.target.checked }))
                          }
                          className="h-4 w-4 rounded border-white/20 accent-[rgb(var(--accent-orange))]"
                        />
                        Startup
                      ...
                      <button
                        type="button"
                        onClick={() => remove(c.id)}
                        className="rounded-xl p-2 text-muted transition hover:bg-red-950/40 hover:text-red-400"
      ...
      <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgba(255,107,0,0.15)] ring-1 ring-[rgba(255,107,0,0.3)]">
            <Plus className="h-4 w-4 text-[rgb(var(--accent-orange))]" />
          </div>
          <h2 className="text-sm font-semibold text-foreground">Add camera</h2>
        </div>
        <input
          className={inputCls}
        ...
        <input
          ...
          placeholder="Source — USB index (0, 1) or rtsp://..."
          value={source}
        ...
        <button
          ...
          onClick={addCamera}
          disabled={adding}
          className="inline-flex items-center gap-2 rounded-xl border border-white/[0.15] px-4 py-2.5 text-sm font-medium text-foreground transition hover:bg-white/[0.06] hover:border-white/25 disabled:opacity-50"
        >
