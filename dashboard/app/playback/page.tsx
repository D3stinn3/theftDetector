...
export default function PlaybackPage() {
  ...
  useEffect(() => {
    refresh();
    const t = window.setInterval(() => { refresh(); }, hasActive ? 1500 : 5000);
      ...
      <section className="space-y-3">
        ...
        {jobs.length === 0 ? (
          ...
          <ul className="space-y-2">
            {jobs.map((j) => {
              const statusIcon =
                j.status === "completed" ? <CheckCircle2 className="h-3.5 w-3.5" /> :
                j.status === "failed"    ? <XCircle className="h-3.5 w-3.5" /> :
                j.status === "running"   ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> :
                                           <Clock className="h-3.5 w-3.5" />;
              const statusColor =
                j.status === "completed" ? "rgba(0,255,190,0.12)" :
                j.status === "failed"    ? "rgba(255,77,0,0.12)" :
                j.status === "running"   ? "rgba(255,107,0,0.12)" :
                                           "rgba(110,120,145,0.15)";
              const statusText =
                j.status === "completed" ? "rgb(0,255,190)" :
                j.status === "failed"    ? "rgb(255,77,0)" :
                j.status === "running"   ? "rgb(255,107,0)" :
                                           "rgb(110,120,145)";
              return (
                <li
                  key={j.id}
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{ background: statusColor, color: statusText, border: `1px solid ${statusText}40` }}
                        >
                          {statusIcon}
                          {j.status}
                        </span>
                    ...
                    <div className="text-right text-xs text-muted">
                      ...
                      <div>{j.alertsCreated} alert{j.alertsCreated !== 1 ? "s" : ""}</div>
                    </div>
                  </div>
              ...
              {playbackAlerts.map((a) => (
                ...
                >
                  ...
                  <img
                    ...
                    alt=""
                    className="h-24 w-auto max-w-full rounded-xl border border-white/[0.06] object-cover"
                  />
